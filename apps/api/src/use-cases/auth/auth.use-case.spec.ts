...
import { AuditLogService } from "apps/audit-log/src/services/audit-log.service";
...
describe("AuthUsecase", () => {
  ...
  let auditLogService: jest.Mocked<ClientProxy>;
  ...
  beforeEach(() => {
    ...
    authUsecase = new AuthUsecase(userRepository,jwtService ,auditLogService );
  ...
  it("deve retornar o access_token e refresh_token ao fazer login com sucesso", async () => {
    ...
    const result = await authUsecase.login(userDto, '127.0.0.1');
  ...
  it("deve lançar UnauthorizedException se o usuário não for encontrado", async () => {
    ...
    await expect(authUsecase.login(userDto, '127.0.0.1')).rejects.toThrow(
